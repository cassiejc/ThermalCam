// Enhanced Thermal Monitoring System JavaScript
// Version 3.5 - ESP32 Real Thermal Camera Support with Persistent Object Storage + Colormap Selection
...
function getGroupsFromVenomBot() {
    // Show loading state
    const button = document.querySelector('button[onclick="getGroupsFromVenomBot()"]');
    const originalText = button.textContent;
    button.textContent = 'Loading...';
    button.disabled = true;

    fetch('/api/get_venom_bot_groups')
    .then(response => response.json())
    .then(data => {
        if (data.status === 'success' && data.groups.length > 0) {
            // Show groups selection modal
            showGroupSelectionModal(data.groups);
        } else {
            showNotification('No groups found or Venom Bot not connected', 'warning');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showNotification('Error getting groups from Venom Bot. Make sure Venom Bot is running.', 'error');
    })
    .finally(() => {
        // Reset button state
...
function testWhatsApp() {
    ...
    .catch(error => {
        console.error('Error:', error);
        showNotification('Error testing WhatsApp. Make sure Venom Bot is running.', 'error');
    })
    .finally(() => {
        // Reset button state
        button.textContent = originalText;
...
function updateStatus() {
    ...
    .then(data => {
        ...
        // Update Venom Bot status
        const venomStatus = document.getElementById('status-venom-bot');
        if (venomStatus) {
            venomStatus.textContent = data.venom_bot_status || 'Unknown';
            venomStatus.className = data.venom_bot_status === 'Connected' ? 'normal' : 'overheat';
        }
        ...
        if (data.overheat_objects && data.overheat_objects.length > 0) {
            overheatAlert.style.display = 'block';
            overheatList.innerHTML = data.overheat_objects.map(obj =>
